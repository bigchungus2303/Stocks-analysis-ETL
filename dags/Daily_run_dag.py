...
def daily_raw_data_load_dag_v1():
    MAANG_STOCKS = [
                ...
                'CCOI','SBRA','DORM','SMPL','ADMA','IRTC','XRAY','PLXS','RNA','NCNO','TFSL','ALVO','POWI','ACHC','ZLAB','TCBI','RXRX','IMVT','HEES','DLO',
                'FULT','EBC','STRL','PCH','SRRK','ASO','PAYO','BHF','LIF','RIOT','CRNX','CAMT','ACLX','RYTM','NMRK','APLS','ARLP','OTTR','OSIS','BTSG','IRDM',
                'SNRE','PRCT','FHB','PTON','MGEE','RUM','ACAD','AVPT','ZI','NWE','BLKB','AMBA','MTSR','CARG','FIBK','CATY','MRUS','PTEN','VEON','WSFS','SGRY',
                'PTVE','SMTC','FIVN','PLMR','FA','ALKT','PENN','MGRC','WDFC','WEN','BL','CAR','VCYT','PATK','XENE','CORZ','RGTI','NFE','MEOH','DOOO','DNLI',
                'ACVA','AMED','LAUR','LFST','ALRM','PRVA','STNE','CNXC','INDB','VICR','GT','NWL','SYNA','CAKE','CVBF','HCM','PI','SHOO','WB','FOLD','PDCO',
                'FORM','NMIH','LUNR','QDEL','SFNC','USLM','MRX','GCMG','IPGP','TOWN','PSMT','TBBK','MLTX','HUBG','YY','WGSWW','WGS','GSAT','DIOD','PLTK','CLSK',
                'TTMI','FFBC','IOSP','CPRX','VIAV','LIVN','VCEL','EWTX','CHEF','MRCY','ALHC','MLCO','BATRA','JJSF','BTDR','MIRM','FRME','ODD','ARWR','HSAI',
                'IBRX','MGNI','JBLU','STRA','APPN','TWST','BATRK','DRVN','CENT','AMRX','BEAM','TMDX','VC','LION','NVCR','PRGS','SBCF','WAFD','KYMR','IREN',
                'APLD','WSBC','MCW','PPBI','PSNY','PSNYW','BANR','PTGX','INTR','LGND','CRTO','LMAT','KLIC','TRMK','RNW','NBTB','CLOV','FLYW','CNTA','EFSC',
                'HWKN','NEOG','IQ','CENTA','MESO','AGYS','SYBT','ENLT','POWL','COCO','CGON','CERT','TRIP','UPWK','GTX','WERN','SSRM','JANX','PWP','ACLS','ZD',
                'NEXT','AZTA','ENVX','VSEC','TNDM','HURN','EXTR','AGIO','RPD','SLNO','NAMS','ARCB','OSW','SIMO','PGNY','HRMY','HLMN','ROCK','APGE','PSEC','ADUS',
                'UFPT','SKWD','MYRG','CIFR','JAMF','TVTX','IDYA','BCRX','MBIN','AVDX','RUN','GOGL','ICFI','ATRC','EVCM','HIMX','MQ','PAX','ADEA','GDRX','CASH','OMCL',
                'IRON','TRMD','LGIH','TARS','SUPN','VERA','AAPG','ULCC','SBLK','INOD','GDYN','CSGS','GLPG','IOVA','BLTE','SJW','ASTH','PLUS','PCT','SABR','LOT','OPRA',
                'HPK','FBNC','CHCO','AMPL','PRDO','CNXN','MTTR','NTCT','PINC','WVE','IAS','UCTT','HUT','STGW','LKFN','DVAX','LUNRW','SDGR','MCRI','TWFG','ACMR','SPT','IART',
                'PLYA','CENX','CLBK','SEZL','NWBI','XMTR','EH','MNKD','ARHS','EXPI','EVO','TFIN','INFN','SRCE','AFYA','WULF','SPNS','PZZA','DNUT','GERN','CMPO','ETNB','NN','LX',
                'ANDE','TIGR','VITL','UPBD','ARQT','GO','FORTY','MLKN','ROOT','VRNT','PLUG','STBA','ATEC','AMPH','LZ','IMCR','CLDX','DMLP','NYAX','HDL','ATSG',
                'TCBK','ALGT','KNSA','UDMY','KARO','NAVI','PRAX','VECO','SASR','NEO','DYN','SONO','VBTX','DXPE','MXL','MOMO','TRUP','SNDX','BUSE','PLAB','UNIT',
                'MFIC','GRAL','LQDA','CRAI','LILA','GPCR','LILAK','DSGR','WBTN','HELE','CCB','AAOI','FTRE','DCOM','TXG','DSP','OCSL','VIR','HOPE','INMD','WABC',
                'SGML','QCRH','NMFC','MYGN','VRDN','DAVE','CRCT','TASK','CLMT','HTZ','NRIX','NVAX','RBCAA','FLNC','DAWN','TILE','SYRE','TBLA','DGII','NTLA',
                'CRSR','BLFS','CMPR','GIII','HLIT','FWRG','ADPT','ARVN','WLFC','IMKTA','IMKT.A','MRTN','ARDX','WINA','PLSE','CDNA','KRNT','RXST','DCBO','ECPG',
                'CSWC','ACDC','MDXG','GABC','VSAT','PCRX','PFBC','NVEE','SAFT','IGIC','KALU','FSUN','XPEL','PENG','QNST','AUPH','ICHR','DGNX','INVA','ANIP',
                'OPK','BLBD','HEPS','AHCO','PEBO','XNCR','OCUL','RCKT','SPRY','FOXF','INDV','COMM','BELFB','REPL','NNE','PGY','ELVN','LQDT','AMWD','BELFA',
                'CCEC','APOG','TTGT','CBRL','BBSI','GOGO','BRKL','MRVI','OCFC','REAX','HROW','BFC','SEDG','THRM','PTLO','AMSC','PFC','BLMN','AOSL','ARRY',
                'HSTM','AMAL','OPEN','ZYME','GYRE','AIOT','XRX','COHU','EOSE','CTBI','RDFN','QUBT','CNOB','RDWR','SBGI','SLRC','BGM','AMSF','OLPX','COLL',
                'BASE','ESTA','BHRB','TRIN','TH','EYE','TRS','IIIV','FBYD','AVBP','PAHC','PLAY','CGBD','MSEX','FMBH','LMB','NSSC','SEAT','OCS','IPX','ABCL',
                'PHVS','UXIN','TLRY','PDFS','PRAA','VMEO','GDEN','SNCY','ANGI','EOLS','CBLL','COGT','EVGO','BBNX','NESR','AVO','ARKO','SCSC','SION',
                'FUFU','BJRI','UVSP','PNTG','ADTN','JBSS','ERII','HSII','ATLC','NBN','KRUS','PRTA','HTLD','STKL','HCKT','STAA','RBBN','ITRN','OSBC',
                'HFWA','CNCK','EMBC','AVAH','CEVA','AVDL','FWRD','TCPC','NBBK','BCYC','CECO','BMBL','PRTH','WOOF','SSYS','CFB','OPT','AXGN','ASTL',
                'HBT','ADV','FDUS','THRY','IMNM','BFST','GHRS','HCSG','CTLP','MLAB','MBWM','NRDS','GCT','MCBS','CFFN','DH','ADSE','IMOS','NTGR','WRLD',
                'SHLS','MATW','SIBN','CRON','PSIX','EZPW','CRESY','CSIQ','TRNS','DAKT','CVAC','FCBC','SWIM','EVER','HBNC','FNKO','CAC','ULH','ATEX',
                'JACK','ABL','TIPT','CSTL','IRMD','ATRO','HAFC','AVXL','REAL','BCAX','MGPI','ASTE','DNTH','ECX','IBCP','GOOD','PUBM','TYRA','GSBC','MBUU',
                'INDI','EGBN','QURE','OFIX','CCAP','CAPR','FIP','ATGL','RPAY','SANA','TMC','FSBC','RXT','SPTN','SMBC','CTKB','CGNT','PLPC','RLAY','VINP',
                'BVS','GSM','OSPN','ABUS','LENZ','BYRN','UFCS','MOFG','HTBK','GLAD','KURA','ORRF','SLP','SFIX','NCMI','HTBI','VALN','MVST','CLNE','LEGH',
                'TRST','BITF','RCAT','ESQ','KRT','PHAR','LPRO','MGIC','VERV','AMRK','CRMD','WASH','CNDT','LIND','ACIC','RZLV','CCBG','MNTK','ANNA','DHC',
                'SCVL','WEST','THFF','CCRN','FLX','NVTS','SHEN','DMRC','CASS','FRPH','NYMT','PPTA','TMCI','SCHL','HNST','FISI','KIDS','LVRO','RMR','MGTX',
                'PGC','NEXN','ABSI','HIFS','OB','API','GLDD','ANAB','EVLV','ACCD','SVRA','DJCO','MNRO','MNMD','IMXI','PGEN','XERS','GRRR','CLMB','LYTS',
                'SPFI','SENEB','SENEA','TREE','MPB','IMTX','ORIC','ODP','CRNC','CMCO','FMNB','RDVT','OMER','CAN','MAZE','SERV','DGICA','JFIN','CGEM','CCNE',
                'ALRS','VREX','EBTC','DGICB','AUTL','CBNK','METC','MERC','GAMB','BCAL','SHBI','TALK','METCB','LINC','ABVX','OPAL','FARO','FLGT','RSVR',
                'GAIN','CCSI','DADA','SRDX','KALV','RNAC','CADL','DDI','NNDM','GRPN','PFIS','BIGC','TCBX','CELC','NFBK','EU','FLWS','DCGO','LASR','LAB',
                'KELYA','KELYB','SWBI','GCL','ARCT','ZJK','SDA','ZSPC','FFIC','MLYS','YORW','DCTH','LGTY','NWPX','HONE','UNTY','SNDL','VLGEA','SBC','HNRG',
                'BAND','SOHU','RAPP','CMRX','GCBC','RICK','TRDA','SKYT','CLPT','GLRE','ALTI','TBPH','ALT','SMLR','HUMA','ALLO','ANSC','WLDN','INV','GMGI',
                'PACB','CLFD','SAGE','INNV','SVC','NRIM','KE','ALNT','KROS','AIRJ','APPS','KMDA','OABI','OUST','MREO','STOK','AROW','PRCH','PROK','CWCO',
                'TECX','RWAY','FBIZ','EYPT','GILT','CVGW','PRTC','UPB','MXCT','KRNY','URGN','ITIC','ZEUS','SIGA','BLDP','TWNP','MITK','GRVY','MFH','BSRR',
                'ANGO','NETD','PERI','BZAI','CDXC','KBSX','VBNK','TITN','FFWM','MSBI','TRVI','NPCE','GOCO','LMNR','ZVRA','ACTG','NATH','TBRG','ORGO','LAND',
                'CMPX','BTBT','GPRE','ERAS','ALDX','DHIL','TKNO','CARE','BMRC','AKBA','HRZN','SHYF','CMPS','HIVE','WALD','NYXH','BWB','PHAT','CCIX','HBCP',
                'ORKA','JYD','ATXS','GLUE','KFII','WTBA','AIP','GDEV','PRME','USCB','HAIN','BSVN','APEI','AURA','ETON','RRBI','KLTR','EWCZ','AHG','FRBA',
                'CDRO','VALU','RDUS','OLMA','GPAT','LSAK','NNOX','ZIMV','ASLE','OFLX','ALF','CRMT','BLZE','MBAV','BWMN','MBX','TBCH','ATYR','PBPB','MTRX',
                'CVRX','NRC','RGNX','ESPR','HIT','NVEC','TCMD','CWBC','DOGZ','LOCO','RIGL','LUNG','LAES','BBCP','EGHT','GEVO','CCIR','LE','MTLS',
                'ACNB','AUDC','POET','CDZI','FMAO','NXXT','ASPI','AMCX','TERN','CDXS','WEYS','AEHR','PMTS','IBEX','SATL','PANL','TTSH','GIG','PSNL',
                'LOVE','ATAI','SPOK','ARTNA','PNRG','NEWT','TSHA','JAKK','LWAY','BRY','MVIS','ZBIO'
                ]
    ...
    staging_table = production_table + '_stg_' + current_date_str + '_v6'
    cumulative_table = f'{schema}.your_table_name'
    yesterday_ds = '{{ yesterday_ds }}'

    # Example date range
    START_DATE_STR = date.today().strftime('%d-%m-%Y') # "DD-MM-YYYY" format
    END_DATE_STR   = tomorrow_date.strftime('%d-%m-%Y')
    # START_DATE_STR = '25-02-2025' # "DD-MM-YYYY" format
    # END_DATE_STR   = '26-02-2025'

    def create_iceberg_table_if_not_exists(catalog_obj):
        """
        Check if the staging Iceberg table exists; if not, create it.
        """
        try:
            catalog_obj.load_table(staging_table)
    ...
    def append_data_to_iceberg(catalog_obj, arrow_table: pa.Table) -> None:
        """
        ...
        try:
            table = catalog_obj.load_table(staging_table)
    ...
    def main():
        ...
        type="",
        uri="",
        warehouse=CATALOG_NAME,  # Adjust secret name
        credential=tabular_credential,
        )
        create_iceberg_table_if_not_exists(catalog_obj)
        ...
        append_data_to_iceberg(catalog_obj,big_arrow_table)


    def run_snowflake_dq(query: str) -> bool:
        """
    ...
    dbt_build_whole_project = BashOperator(
        ...
        bash_command='export DBT_SCHEMA="acampi" && cd /usr/local/airflow/capstone-dbt_2 && dbt deps && dbt build'
    )        
    #
    # Airflow tasks below
    #
    ...
    run_dq_check = PythonOperator(
        ...
        python_callable=run_snowflake_dq,
        op_kwargs={
            'query': f"""
                SELECT * 
    ...
    create_daily_step >> create_staging_step >> load_to_staging_step >> load_to_production_step >> drop_staging_table >> run_dq_check >> cleaning_step >> dbt_build_whole_project 
